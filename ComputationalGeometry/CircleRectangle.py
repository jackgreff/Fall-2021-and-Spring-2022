...
Jack Greff (I collaberated with Scott Kaiser and Daniel Rosman. Scott helped with some of the )
...
def CRO(centerX: float,centerY: float,radius: float,xmin: float,xmax: float,ymin: float,ymax: float) -> bool:
    ...
    rcenterX = (xmin + xmin)/2
    rcenterY = (ymin + ymax)/2

    rad_x = (xmax - xmin)/2
    rad_y = (ymax - ymin)/2

    centers_dist_x = abs(centerX - rcenterX)
    centers_dist_y = abs(centerY - rcenterY)

    # centers_dist = math.sqrt(centers_dist_x**2 + centers_dist_y**2)
    # rad =  math.sqrt(rad_x**2 + rad_y**2)

    centerCenterDist = math.sqrt((rcenterX-centerX)**2 + (rcenterY-centerY)**2)
    diagonalDistance = math.sqrt((xmax-rcenterX)**2 + (ymax-rcenterY)**2)


    if (centers_dist_x < rad_x + radius and centers_dist_y < rad_y + radius and centerCenterDist <= radius+diagonalDistance):
        return True
    else:
        return False
