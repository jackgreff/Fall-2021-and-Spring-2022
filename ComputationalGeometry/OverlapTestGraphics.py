...
def do_rangeOverlap(min1,max1,min2,max2):
    from Range import rangeOverlap
    return rangeOverlap(min1,max1,min2,max2)

def doWindowOverlap(minx1,maxx1,miny1,maxy1,minx2,maxx2,miny2,maxy2):
...
def doCircleOverlap(x1,y1,r1,x2,y2,r2):
    from Circle import circleOverlap
    print("Testing: >>> circleOverlap(", x1, ", ", y1, ", ", r1, ", ", x2, ", ", y2, ", ", r2, ")")
    return circleOverlap(x1,y1,r1,x2,y2,r2)

def doCircleRectangleOverlap(center_x,center_y,radius,xmin,xmax,ymin,ymax):
    from CircleRectangle import circleRectangleOverlap
    print("Testing: >>> circleRectangleOverlap(", center_x, ", ", center_y, ", ", radius, ", ", xmin, ", ", xmax, ", ", ymin, ", ", ymax, ")")
    return circleRectangleOverlap(center_x,center_y,radius,xmin,xmax,ymin,ymax)

def doSegmentOverlap(x1, y1, x2, y2, x3, y3, x4, y4):
...
def greaterOfTwo(int1,int2):
    if(int1 > int2):
        return int1
    else:
        return int2

def lesserOfTwo(int1,int2):
    if(int1 < int2):
        return int1
    else:
        return int2
...
class App:
    ...
    def playback(self):
        ...
        if(func[0] == "rangeOverlap"):
            ...
            if(do_rangeOverlap(int(func[1]),int(func[2]),int(func[3]),int(func[4]))):
            ...
            if(doWindowOverlap(int(func[1]),int(func[2]),int(func[3]),int(func[4]),int(func[5]),int(func[6]),int(func[7]),int(func[8]))):
        ...
        elif(func[0] == "circleOverlap"):  # Note #3 and #6 are radii, i.e. may be non-integer
            ...
            if(doCircleOverlap(int(func[1]),int(func[2]),float(func[3]),int(func[4]),int(func[5]),float(func[6]))):
        ...
        elif(func[0] == "circleRectangleOverlap"):  # Note #3 is radius, i.e. may be non-integer
            ...
            if(doCircleRectangleOverlap(int(func[1]),int(func[2]),float(func[3]),int(func[4]),int(func[5]),int(func[6]),int(func[7]))):
            ...
            if(doSegmentOverlap(int(func[1]),int(func[2]),int(func[3]),int(func[4]),int(func[5]),int(func[6]),int(func[7]),int(func[8]))):
...
#================================================================================
    ...
    def endflatline(self, event):
        ...
        self.temp = self.canvas.create_line([self.startx,self.starty,event.x,self.starty],fill="yellow")
        if(self.min1):
            self.max2 = greaterOfTwo(self.startx,event.x)
            self.min2 = lesserOfTwo(self.startx,event.x)
            self.cleartext()
            if(do_rangeOverlap(self.min1,self.max1,self.min2,self.max2)):
                self.temptext = self.frame3.create_text(100,50,text="Range Overlap!")
            else:
                self.temptext = self.frame3.create_text(100,50,text="No Range Overlap")
            if(self.record.get()):
            ...
            self.max1 = greaterOfTwo(self.startx,event.x)
            self.min1 = lesserOfTwo(self.startx,event.x)
...
#================================================================================
    ...
    def endwindow(self, event):
        ...
        self.temp = self.canvas.create_rectangle([self.startx,self.starty,event.x,event.y],outline="green")
        if(self.minx1):
            self.maxx2 = greaterOfTwo(self.startx,event.x)
            self.minx2 = lesserOfTwo(self.startx,event.x)
            self.maxy2 = greaterOfTwo(self.starty,event.y)
            self.miny2 = lesserOfTwo(self.starty,event.y)
            self.cleartext()
            if(doWindowOverlap(self.minx1,self.maxx1,self.miny1,self.maxy1,self.minx2,self.maxx2,self.miny2,self.maxy2)):
                self.temptext = self.frame3.create_text(100,50,text="Window Overlap!")
            else:
                self.temptext = self.frame3.create_text(100,50,text="No Window Overlap")
            if(self.record.get()):
            ...
            self.maxx1 = greaterOfTwo(self.startx,event.x)
            self.minx1 = lesserOfTwo(self.startx,event.x)
            self.maxy1 = greaterOfTwo(self.starty,event.y)
            self.miny1 = lesserOfTwo(self.starty,event.y)
...
#================================================================================
    ...
    def endcircle(self, event):       
        ...
        if(self.x1):
            ...
            self.r2 = difference
            self.cleartext()
            if(doCircleOverlap(self.x1,self.y1,self.r1,self.x2,self.y2,self.r2)):
                self.temptext = self.frame3.create_text(100,50,text="Circle Overlap!")
            else:
                self.temptext = self.frame3.create_text(100,50,text="No Circle Overlap")
            if(self.record.get()):
                file = open("stored_tests.py", "a")  # Dave W took out "int" for radii Sept 2011
                file.write(">>> circleOverlap(" + str(self.x1) + ", " + str(self.y1) + ", " + str(self.r1) + ", " + str(self.x2) + ", " + str(self.y2) + ", " + str(self.r2) + ")\n")
                file.close()
            self.clearcanvas()
        else:
            self.x1 = self.startx
...
#================================================================================
    ...
    def endcircrect(self, event):
        if(self.minx1):
            ...
            if(doCircleRectangleOverlap(self.x2,self.y2,self.r2,self.minx1,self.maxx1,self.miny1,self.maxy1)):
                self.temptext = self.frame3.create_text(100,50,text="Circle Rectangle Overlap!")
            else:
                self.temptext = self.frame3.create_text(100,50,text="No Circle Rectangle Overlap")
            if(self.record.get()):
                file = open("stored_tests.py", "a")  # dave W took out "int" for radius Sept 2011
                file.write(">>> circleRectangleOverlap(" + str(self.x2) + ", " + str(self.y2) + ", " + str(self.r2) + ", " + str(self.minx1) + ", " + str(self.maxx1) + ", " + str(self.miny1) + ", " + str(self.maxy1) + ")\n")
                file.close()
            self.clearcanvas()
        else:
            self.canvas.delete(self.temp)
...
#================================================================================
    ...
    def endsegment(self, event):
        ...
        if(self.minx1):
            ...
            self.cleartext()
            if(doSegmentOverlap(self.minx1,self.miny1,self.maxx1,self.maxy1,self.minx2,self.miny2,self.maxx2,self.maxy2)):
                self.temptext = self.frame3.create_text(100,50,text="Segment Overlap!")
            else:
                self.temptext = self.frame3.create_text(100,50,text="No Segment Overlap")
            if(self.record.get()):
